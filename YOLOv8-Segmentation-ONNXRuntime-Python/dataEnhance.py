...
class DataAugmentation(object):
    ...
    def resize_(self, img, boxes, size=(400, 400)):
        """
    ...
    def center_crop(self, img, boxes, size=(random.randint(800, 1600), random.randint(800, 1600))):
        """
        中心裁剪
        :param img: Image
        :param boxes: bbox坐标
    ...
    def random_crop(self, img, boxes, size=(random.randint(800, 1600), random.randint(800, 1600))):
        """
        中心裁剪
        :param img: Image
        :param boxes: bbox坐标
        ...
        """
        w, h = img.size
        ow, oh = random.randint(round(w / 2), w), random.randint(round(h / 2), h)
        max_size = torch.as_tensor([ow - 1, oh - 1], dtype=torch.float32)
        i = random.randint(0, h - oh)
        j = random.randint(0, w - ow)
        img = img.crop((j, i, j + ow, i + oh))
        label, boxes = boxes[:, :1], boxes[:, 1:5]
        boxes = boxes - torch.Tensor([j, i, j, i])
        boxes = torch.min(boxes.reshape(-1, 2, 2), max_size)
    ...
    def random_rotate(self, img, boxes, degrees=45, expand=False, center=None, fill=0, resample=None):
...
#     create_datasets(method=meth, extimes=3, path=r"D:\selectImg\防风")
